...
namespace QFramework
{
    public class TableReadThreadWork
    {
        class TableInfo
        {
            ...
            public TableInfo(QFramework.DTableOnParse parseRun, string fileName)
            {
        ...
        private static ReadParams CreateReadParams(QFramework.DTableOnParse onParse, string tableName)
        {
        ...
        public void AddJob(string tableName, QFramework.DTableOnParse onParse = null)
        {
            if (onParse == null)
            {
                return;
        ...
        {
            //初始化 避免在子线程调用mono
            FileMgr.Instance.InitStreamingAssetPath();
            IsDone = false;
            m_FinishedCount = 0;
        ...
        private void Work()
        {
            ...
            while (m_RequestFilePathQueue.Count > 0)
            {
                try
                {
                    ...
                    byte[] fileData = FileMgr.Instance.ReadSync(readparm.filePath);
...
*/
        ...
        public static void ReadSync( QFramework.DTableOnParse onParse, string fileName)
        {
            var readParam = CreateReadParams (onParse ,fileName);
            byte[] fileData = FileMgr.Instance.ReadSync(readParam.filePath);
