...
public class ThreadEvent {
	public QFrameworkMsg Key;
    public List<object> evParams = new List<object>();
}

public class NotiData {
	public QFrameworkMsg evName;
    public object evParam;

	public NotiData(QFrameworkMsg name, object param) {
        this.evName = name;
        this.evParam = param;
    }
}
            ...
            while (true) {
                lock (m_lockObject) {
                    if (events.Count > 0) {
                        ...
                        try {
                            switch (e.Key) {
							case QFrameworkMsg.UPDATE_EXTRACT: {     //解压文件
                                    OnExtractFile(e.evParams);
                                }
                                break;
							case QFrameworkMsg.UPDATE_DOWNLOAD: {    //下载文件
